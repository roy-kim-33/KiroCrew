...
- `memory_recall` — search the memory store bound to this session by meaning: Global V1 for an ordinary session, this Crew Member's private V2 store for a member session. It returns distilled facts, lessons and experiences with their sources, not conversation text. Ask it a specific question when a task needs an earlier decision, preference or event; the injected memory block is built once at session start, so recall is how you catch up after the topic moves on. It never reaches another member's store.
- `search_chat_history` / `get_chat_session` / `list_sessions` — recover the exact words of a past conversation: keyword-search your own past transcripts, read one promising `session_key` in full, and browse what work is in flight. Reach for it when `memory_recall` gives the conclusion but you need the verbatim evidence, or when the fact was never distilled into memory. All three are reads and never modify memory.
