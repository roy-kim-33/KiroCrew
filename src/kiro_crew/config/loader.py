...
class KiroCrewConfig:
    ...
    def acp_effective_model(
        ...
        # Fork: with a custom base URL (a router), "auto" must never reach the
        # backend -- the adapter resolves it to its own Bedrock default
        ...
        if self.agent.acp_backend == ACP_BACKEND_CLAUDE:
            ...
            # "claude_code"). With a router the id is that router's own
            # namespace (cmc/…, oc/…), which a Bedrock-form global.anthropic.*
            # id would break. Without one, native Claude Code takes Anthropic's
            # own short names ("opus", "sonnet", "default") and rejects the
            # Bedrock form outright -- picking Opus became
            # global.anthropic.claude-opus-4-8[1m] and came back "Invalid value
            # for config option model", making the two most-used models
            # unselectable (verified live).
            #
            # Kept as an explicit branch rather than folded into upstream's
            # capability lookup below: the obvious way to express it there would
            # be to make model_registry_namespace() answer "acp" for claude, but
            # that value ALSO keys the advertised-model cache and
            # seed_available_models, so changing it would silently re-key both.
            return m
