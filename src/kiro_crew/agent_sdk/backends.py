...
ACP_BACKEND_ROUTING: dict = {
    ...
    # Fork (RoyCrew): opencode is UNVERIFIED on purpose, and named here so the
    # verdict reads as a decision rather than an id someone forgot to register.
    # Crew does write opencode a native config file
    # (``AcpClient._write_opencode_provider_config``), but it carries PROVIDER
    # CREDENTIALS ONLY -- baseURL, apiKey, wire format, model list -- and no
    # permission settings, so SEEDED_SETTINGS would assert a mechanism this core
    # does not actually operate. Nothing here is enforced on: ENFORCED_ROUTINGS is
    # {SESSION_CONFIG}, and INDETERMINATE is the same verdict claude's
    # SEEDED_SETTINGS already returns, so this reports the gap honestly in doctor
    # and the dashboard without refusing the session.
    ACP_BACKEND_OPENCODE: Routing.UNVERIFIED,
}
