...
def _literal_is_not_public(
    ...
    * 6to4 is checked on BOTH readings, not unwrapped. ``2002:0a00:0001::1`` is a
    ...
    # Folded in here (rather than left in the old single caller) so BOTH
    # entry points that now share this function — address_is_not_public and
    # _reject_if_internal_ip — get the same interpreter floor.
