...
_EDITABLE_CONFIG: dict[str, dict] = {
    ...
    # Anthropic internally). A harness is selected here, never via
    # agent.provider (locked to "acp" above — harness-parity).
    #
    # ``values_fn`` rather than a literal list, because the set WIDENS after
    # this module is imported: an edition (or this fork) registers a backend
    # from ``ProviderRegistry.register_acp_backends`` /
    # ``acp_backends.register_selectable_backend`` at boot, and a frozen
    # literal here would reject it with a misleading "invalid value".
    ...
    # claude backend that is the native lane (no ANTHROPIC_BASE_URL is
...
async def api_kirocrew_config_patch(request: web.Request) -> web.Response:
    ...
    if spec["type"] == "enum":
        ...
        # when an edition — or this fork — registers a backend. A static ``values``
        # list would be read before that happened.
