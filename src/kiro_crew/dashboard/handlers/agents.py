...
def _advertised_cc_models(request: web.Request) -> list[dict]:
    ...
    Fork: the advertised id is used VERBATIM, never remapped through the
    registry's ``from_provider_id``. That remap folds Claude Code's own short
    config-option aliases (``opus``, ``sonnet``) onto Bedrock-flavored canonical
    keys meant for a DIFFERENT id shape, which a native claude_code session
    never advertises -- so selecting the remapped row sent a value the adapter
    rejects with "Invalid value for config option model" (verified live; those
    are the two most-used non-default models and both were unselectable).
    """
    try:
        state: DashboardState = request.app["state"]
        ...
        return []
    # Fork: reversed() so the NEWEST session wins. Iterating oldest-first made a
    # provider switch keep serving the previous backend's advertised list until
    # every old session aged out, which is what left the picker showing the
    # router's models on the native lane (and vice versa) until a reload.
    # Upstream's capability gate is kept on top of that ordering.
...
async def api_models(request: web.Request) -> web.Response:
    """GET /api/models — the live ACP session's models, per backend."""
    # Fork: the claude (router) and opencode lanes each need their own responder
    # -- the picker must show the router's real catalog, not a Bedrock list --
    # so these keep the fork's async responders rather than upstream's sync
    # _cc_models(). Upstream's off-loop config read is adopted.
    cfg = await asyncio.to_thread(KiroCrewConfig.load)
    if getattr(cfg.agent, "acp_backend", "") == ACP_BACKEND_CLAUDE:
        return await _cc_models_response(request)
    if getattr(cfg.agent, "acp_backend", "") == ACP_BACKEND_OPENCODE:
...
async def api_kirocrew_agents_create(request: web.Request) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        model_reason = _model_pin_rejected(model, request, getattr(cfg.agent, "acp_backend", ""))
        if model_reason:
