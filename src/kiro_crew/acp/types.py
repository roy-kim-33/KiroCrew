...
# ── Fork addition: the opencode CLI backend ──
# Imported from ``acp_backends`` with the others above, not defined here: that
# leaf module is the DEFINING owner of every backend id, and opencode is
# registered there in both ``ACP_BACKENDS_KNOWN`` (so provider construction
# accepts it) and ``BASELINE_SELECTABLE_BACKENDS`` (so ``agent.acp_backend``
# admits it via ``resolve_selected_backend``, which reads that registry rather
# than this module). Re-exported here because the fork's session/client/provider
# code imports it from this, its pre-refactor home.
