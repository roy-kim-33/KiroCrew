...
class AcpSessionHandle:
    ...
    async def prompt(self, message: str, timeout: float | None = None) -> AsyncIterator[AcpEvent]:
        ...
        async def _build() -> tuple[str, dict[str, Any]]:
            # Fork: text-only router models (the deepseek-v4-flash family)
            # reject image content upstream. When the active model routes
            # images to text, describe each referenced image via the vision
            # chain and inject its description, so no image block ever reaches
            # the rejecting upstream. Mirrors AcpClient._send_prompt so the
            # shared-runtime path (chat/dashboard/cron) behaves like the
            # direct-client path.
            #
            # `nonlocal` because upstream moved the send path into the shared
            # _run_turn(build_request), which has no `message` in scope: the
            # rewrite has to land on prompt()'s own variable, which this
            # closure is what still sees.
            nonlocal message
            if _message_has_image_path(message):
                ...
                main_backend = (
                    a.acp_backend
                    if a.acp_backend in (ACP_BACKEND_CLAUDE, ACP_BACKEND_OPENCODE)
                    else ""
                )
                from kiro_crew.acp.vision import redirect_image_message

                message, _image_mode = await redirect_image_message(
                    message,
