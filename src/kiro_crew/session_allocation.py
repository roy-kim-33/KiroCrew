...
def _model_is_text_only_for_session(cfg: Any, model: str | None) -> bool:
    ...
    async def get_or_create(
        ...
        **extra_factory_kwargs: Any,
        ...
        if pooled is not None:
            ...
            # Fork: text-only image guard. A session resuming on a text-only
            # router model would replay any image blocks the SDK stored in its
            # history (from before a model switch, or a previous vision-model
            # session), and the text-only upstream rejects them with a 400.
            # Discard the resume so the session starts fresh on the text-only
            # model; conversation replay then carries only text.
            if resume_sid and _model_is_text_only_for_session(self._deps.load_config(), model):
                self._deps.logger.info(
