...
def _doctor(platform_boot_error: "Exception | None" = None, bundle: bool = False) -> None:
    ...
    # Claude Code is selectable too (``BASELINE_SELECTABLE_BACKENDS``); this fork
    # additionally lets a deployment CONFIGURE it as the active backend via
    # agent.acp_backend (see harness-parity: a harness is selected at
    # acp_backend, never a second agent.provider value). When it is the active
    # backend, kiro-cli's absence is not an issue and claude-acp's absence is —
    # so the required/optional framing below flips on that config read.
    _cfg_backend: str = ""
    try:
        _cfg_backend = KiroCrewConfig.load().agent.acp_backend
    except Exception:
        pass
    ...
    if _backend_is_claude:
        ...
        # the real blocker -- kiro-cli's absence above is not.
        # `_doctor_claude_backend()`'s own framing always reports Claude Code
        # as an optional backend, with no "this is the active one" state, so
        # that case is reported directly here, through the SAME probe it uses
        # (``agent_sdk.probe_backend``) so doctor and the dashboard still
        # cannot disagree about whether the adapter is installed.
        try:
            ...
            claude_state = None
        if claude_state is not None and claude_state.installed == INSTALLED:
            where = shutil.which(_CLAUDE_ACP_BIN)
            print(f"  claude-acp:  ✅ {where or 'resolved off PATH'} (active backend)")
        elif claude_state is not None:
            missing = ", ".join(claude_state.missing_components) or "components"
            print(
                f"  claude-acp:  ❌ {missing} not found "
                "(active backend for agent.acp_backend=claude)"
            )
            if claude_state.install_command:
                print(f"               {claude_state.install_command}")
        else:
