...
// Stub the gate's three API calls. The status answer keeps the gate closed
// (unauthenticated); the device answer is the frozen session the device
// scene renders.
const deviceSession = {
...
api.kasLoginStatus = () =>
  Promise.resolve({ authenticated: false, identities: [] }) as ReturnType<
    typeof api.kasLoginStatus
  >
...
// The device scene is reached through the REAL wiring: the harness clicks the
// shipped Google button once the chooser is on screen.
if (scene === 'device') {
  const t = setInterval(() => {
