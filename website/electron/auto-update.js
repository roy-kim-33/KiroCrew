...
// RoyCrew fork: the update feed is this fork's OWN GitHub releases
// (``DOWNLOAD_BASE`` below), NOT upstream's distribution CDN. Upstream's comment
// here named updates.crew.kiro.dev / download.crew.kiro.dev; pointing this fork
// at those would make it update itself into upstream's build. The mechanism
// below is unchanged -- the fork's release publishes latest-mac.yml and
// latest-linux.yml alongside its artifacts, so the generic provider resolves
// them from the releases directory exactly as described.
//
