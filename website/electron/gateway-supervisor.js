...
function createGatewaySupervisor({
  ...
  // Fork: /api/health can stay healthy after a Linux AppImage shell exits while
  // its backend survives under an unmounted /tmp/.mount_* path. Probe the
  // dashboard root too: that request touches the bundled static files and
  // exposes the stale mount as HTTP 500 before Electron decides to reuse the
  // process. Feeds decideGatewayAction's `gatewayUsable`, whose "restart-local"
  // verdict already lives in instance-guard.js.
  ...
  async function resolveGatewayConflict(rebindDepth = 0) {
    ...
    // A remote tunnel's root is not ours to judge, so it is usable by definition.
    ...
    if (decision.action === "restart-local") {
      // Fork: our own gateway answers /api/health but cannot serve the
      // dashboard (the stale-AppImage-mount case). Reusing it strands the
      // shell on a process that will never render, so replace it.
