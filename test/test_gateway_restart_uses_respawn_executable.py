...
class TestAutoApplyGitUpdate:
    ...
    def _scripted_git(monkeypatch) -> AsyncMock:
        ...
        async def _spawn(*argv, **_kwargs):
            ...
            if words[:2] == ["config", "--get"]:
                # This fork resolves the remote the branch actually tracks
                # instead of assuming "origin", so the pin, the fetch and the
                # reset below all name the same remote.
                return _proc(0, b"origin\n")
            if words[:1] == ["fetch"]:
                return _proc(0)
    ...
    async def test_successful_update_execs_the_respawn_interpreter(
        ...
        monkeypatch.setattr(gw, "tracks_upstream", lambda _proj, _branch, remote="": True)
