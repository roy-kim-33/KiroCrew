...
# This fork publishes no Windows desktop release, so it removed the NSIS
# installer job from `build.yml` rather than smoke-installing an artifact it
# never ships; the removal is stated in the workflow itself. Upstream's two
# shape assertions below therefore have no job to read here. They are made
# conditional on the job EXISTING rather than deleted, so re-adding an installer
# job — on this fork or on a future re-sync — is still held to upstream's
# contract instead of landing unchecked. The doc-honesty tests that follow are
# the point of this file and run unconditionally on both shapes.
_REMOVAL_MARKER = "Windows installer build REMOVED"


def _installer_job_removed(workflow: str) -> bool:
    return _REMOVAL_MARKER in workflow
...
def test_the_installer_job_skips_gateway_validation(workflow: str) -> None:
    ...
    if not invocations:
        assert _installer_job_removed(workflow), (
            "build.yml no longer invokes the Windows installer script, and does "
            f"not say why: expected the {_REMOVAL_MARKER!r} rationale"
        )
        return
    assert all(
...
def test_the_synthetic_payload_is_why_it_is_skipped(workflow: str) -> None:
    ...
    if _installer_job_removed(workflow):
        pytest.skip("this fork ships no Windows desktop release; job removed")
