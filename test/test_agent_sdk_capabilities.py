...
KNOWN_REMAINING_IDENTITY_READS = {
    # RoyCrew fork: these branches select the CLAUDE HARNESS, which in this fork
    # cannot be asked as a capability. Upstream reaches the same answer through
    # ``agent.provider``, but this fork pins ``agent.provider`` to "acp" for
    # upstream harness parity, so the identity lives only on ``agent.acp_backend``
    # and a capability lookup returns the wrong lane.
    #
    # ``acp_effective_model`` is the sharpest case: the obvious capability
    # (``capabilities_for(...).model_id_namespace``) resolves through
    # ``model_registry_namespace()``, and that same value ALSO keys the
    # advertised-model cache and ``seed_available_models`` -- so bending it to
    # answer this question would silently re-key both. The branch stays explicit
    # and the coupling stays visible.
    "dashboard/handlers/agents.py": {"api_models", "api_slash_commands", "_model_pin_rejected"},
    "config/loader.py": {"acp_effective_model", "create_provider_factory"},
    "dashboard/chat_handlers.py": {"_model_rejected_reason"},
    "dashboard/chat_runner.py": {"_run_chat"},
}
...
def test_known_membership_is_unchanged_by_the_move() -> None:
    ...
    """
    # RoyCrew fork: "opencode" is a backend THIS fork ships (acp/client.py owns its
    # spawn path and the adapter is a public npm package), so the gate must accept
    # it. The invariant this test protects -- that the move neither gained nor lost
    # an id -- still holds; the expected set just includes the fork's own.
...
class TestTheKiroConstructionPathIsUnconditional:
    ...
    def test_the_claude_backend_is_not_translated_in_this_fork(self, monkeypatch) -> None:
        """RoyCrew fork: the claude arm performs NO registry translation.

        Upstream asserts ``to_provider_id(..., "claude_code")`` here. This fork
        returns the id untouched on BOTH claude lanes, which is live-verified
        behaviour, not an oversight: with a router the id is that router's own
        namespace (cmc/..., oc/...), and without one native Claude Code takes
        Anthropic's short names and answers the Bedrock form with "Invalid value
        for config option model" -- which made the two most-used models
        unselectable. See ``acp_effective_model``'s own comment.

        The kiro/kas/codex arm is unchanged and still covered by the sibling
        tests, so the "each backend reaches its own namespace" invariant is
        intact; only the claude arm's destination differs in this fork.
        """
