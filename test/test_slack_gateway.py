    ...
    *,
    ...
    async def _fake(*args, **kwargs):
        ...
        if "rev-parse" in argv and "--abbrev-ref" in argv:
            ...
            # Fork: `branch.<name>.remote`, so a checkout of a fork updates from
            # ITS OWN remote rather than a hardcoded "origin". Empty `remote`
            # (the default) mirrors an unset config -- the read falls back to
            # "origin" in the caller.
            out = remote
        elif "fetch" in argv:
            rc = fetch_rc
...
class TestAutoApplyUpdateGitPath:
    ...
    @pytest.mark.asyncio
    async def test_fetch_uses_the_tracked_remote_not_origin(self):
        """Fork: fetch/reset target `branch.<name>.remote`, not a hardcoded origin.

        A fork checkout's branch tracks its own remote (e.g. "fork"), not
        upstream's "origin". Resolving the remote name from git config -- and
        threading that name into the fetch -- is what lets `_auto_apply_update`
        update a fork from the remote it actually tracks.
        """
        orch = _make_orchestrator()
        ds = _mock_dashboard_state()
        orch.dashboard_state = ds
        ...
        record: list = []
        _fake_exec = _git_exec_fake(remote=b"fork\n", diff_rc=0, record=record)
        ...
        fetch_calls = [args for args in record if "fetch" in args]
        assert fetch_calls, record
        assert all("fork" in args for args in fetch_calls), record
        assert not any("origin" in args for args in record), record

    @pytest.mark.asyncio
    async def test_fetch_falls_back_to_origin_when_untracked(self):
        """No `branch.<name>.remote` configured -> the historical "origin" default."""
        orch = _make_orchestrator()
        ds = _mock_dashboard_state()
        orch.dashboard_state = ds

        record: list = []
        _fake_exec = _git_exec_fake(diff_rc=0, record=record)  # remote="" (unset)
        ...
        fetch_calls = [args for args in record if "fetch" in args]
        assert fetch_calls, record
        assert all("origin" in args for args in fetch_calls), record
