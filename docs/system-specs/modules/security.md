...
**Pooled-backend declared-env forwarding (`mcp_gateway.forward_declared_env`, default ON)** — an agent spec may declare `mcpServers.<name>.env`. Under pooling one backend serves many sessions, so the rewriter writes that block to a `0600` sidecar and the stub folds it into the `effective_env_hash` PoolKey dimension. With the flag ON, `gatewayd` reads the sidecar at **cold spawn only** and applies the surviving keys, filtered twice:
